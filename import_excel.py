...
def detect_columns(df: pd.DataFrame) -> Dict[str, str]:
    """自动识别列名"""
    return detect_named_columns(list(df.columns))
    ...
    *,
    ...
    if missing_cols:
        if auto_confirm:
            return {
                "success": False,
                "error": f"无法自动识别必需列: {', '.join(missing_cols)}",
            }
        ...
        for col_name in missing_cols:
            while True:
                try:
                    col_idx = (
                        int(input(f"   {col_name} 对应的列号 (1-{len(df.columns)}): "))
                        - 1
                    )
                    if 0 <= col_idx < len(df.columns):
                        column_mapping[col_name] = str(df.columns[col_idx])
                        break
                    else:
                        print("   ❌ 列号超出范围")
                except ValueError:
    ...
    # 确认导入
    print(f"\n📋 即将导入 {len(df)} 条记录")
    confirm = "y" if auto_confirm else input("确认导入? (y/n): ").strip().lower()
    ...
    for idx, row in df.iterrows():
        row_number = int(idx) + 2 if isinstance(idx, int) else 2
        try:
            # 解析日期
            ...
            failed_records.append({"row": row_number, "reason": f"处理错误: {str(e)}"})
...
def save_failed_log(failed_records: List[Dict[str, Any]]):
    """保存导入失败的记录日志"""
